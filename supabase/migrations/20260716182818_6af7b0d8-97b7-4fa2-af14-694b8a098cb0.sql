-- 1) Add per-service referral override on leads
ALTER TABLE public.leads
  ADD COLUMN IF NOT EXISTS referral_name text;

-- 2) Update auto_create_referral_commission to prefer lead.referral_name
...
-- 3) Same fallback logic in ensure_commission_for_confirmed_payment
...
BEGIN
  ...
  SELECT COALESCE(NULLIF(trim(l.referral_name), ''), NULLIF(trim(ct.referral_name), ''))
    INTO v_referral
