CREATE OR REPLACE FUNCTION public.merge_contacts(p_source_contact_id uuid, p_target_contact_id uuid, p_update_phone boolean DEFAULT false, p_update_email boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE c.status IN ('APROVADO', 'ASSINADO', 'REPROVADO')
    AND (
      ...
      OR EXISTS (
        ...
        WHERE o.id = c.opportunity_id AND l.contact_id = p_source_contact_id
      )
      OR EXISTS (
        SELECT 1 FROM contract_leads cl
        JOIN leads l ON l.id = cl.lead_id
        WHERE cl.contract_id = c.id AND l.contact_id = p_source_contact_id
      )
    );
  ...
  WHERE contact_id = p_source_contact_id
    AND NOT EXISTS (
      ...
      WHERE c.status IN ('APROVADO', 'ASSINADO', 'REPROVADO')
        AND (
          EXISTS (SELECT 1 FROM opportunities o WHERE o.id = c.opportunity_id AND o.lead_id = leads.id)
          OR EXISTS (SELECT 1 FROM contract_leads cl WHERE cl.contract_id = c.id AND cl.lead_id = leads.id)
        )
    );
  GET DIAGNOSTICS v_moved_leads = ROW_COUNT;

  SELECT COUNT(*) INTO v_protected_leads FROM leads WHERE contact_id = p_source_contact_id;

  UPDATE interactions SET contact_id = p_target_contact_id WHERE contact_id = p_source_contact_id
    AND (lead_id IS NULL OR lead_id NOT IN (SELECT id FROM leads WHERE contact_id = p_source_contact_id));
  ...
  WITH remapped_links AS (
    SELECT
      CASE WHEN beneficiary_contact_id = p_source_contact_id THEN p_target_contact_id ELSE beneficiary_contact_id END AS beneficiary_contact_id,
      CASE WHEN titular_contact_id = p_source_contact_id THEN p_target_contact_id ELSE titular_contact_id END AS titular_contact_id,
      MIN(created_at) AS created_at
    FROM beneficiary_titular_links
    WHERE beneficiary_contact_id = p_source_contact_id OR titular_contact_id = p_source_contact_id
    GROUP BY 1, 2
  ),
  deleted_links AS (
  ...
  INSERT INTO beneficiary_titular_links (beneficiary_contact_id, titular_contact_id, created_at)
  SELECT rl.beneficiary_contact_id, rl.titular_contact_id, COALESCE(rl.created_at, now())
  ...
  UPDATE contacts SET
    phone = CASE
      WHEN p_update_phone AND v_source.phone IS NOT NULL AND v_source.phone <> '' THEN v_source.phone
      ELSE COALESCE(NULLIF(phone, ''), v_source.phone)
    END,
    email = CASE
      WHEN p_update_email AND v_source.email IS NOT NULL AND v_source.email <> '' THEN v_source.email
      ELSE COALESCE(NULLIF(email, ''), v_source.email)
    END,
  ...
  VALUES (
    ...
    jsonb_build_object('source_contact_id', p_source_contact_id, 'source_name', v_source.full_name, 'source_phone', v_source.phone, 'updated_phone', p_update_phone, 'updated_email', p_update_email),
    jsonb_build_object(
      ...
      'protected_contracts', v_protected_contracts, 'protected_leads', v_protected_leads
    )
  );

  IF v_protected_contracts = 0 AND v_protected_leads = 0 THEN
    DELETE FROM contacts WHERE id = p_source_contact_id;
  END IF;

  RETURN jsonb_build_object(
    'success', true,
    'source_deleted', CASE WHEN v_protected_contracts = 0 AND v_protected_leads = 0 THEN v_source.full_name ELSE NULL END,
    'source_preserved', v_protected_contracts > 0 OR v_protected_leads > 0,
    'protected_contracts', v_protected_contracts,
    'protected_leads', v_protected_leads,
