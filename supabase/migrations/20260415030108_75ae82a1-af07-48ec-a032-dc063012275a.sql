...
BEGIN
  ...
  UPDATE payments SET beneficiary_contact_id = p_target_contact_id WHERE beneficiary_contact_id = p_source_contact_id;
  ...
  -- Beneficiary titular links
  UPDATE beneficiary_titular_links SET titular_contact_id = p_target_contact_id
  WHERE titular_contact_id = p_source_contact_id AND beneficiary_contact_id != p_target_contact_id;
  ...
  -- Auto-fill empty fields on target with source data
  UPDATE contacts SET
  ...
  RETURN jsonb_build_object(
    'success', true, 'source_deleted', v_source.full_name, 'target_id', p_target_contact_id,
    'moved_leads', v_moved_leads, 'moved_interactions', v_moved_interactions,
    'moved_pending_items', v_moved_pending, 'moved_beneficiaries', v_moved_beneficiaries,
    'moved_payments', v_moved_payments, 'moved_suggestions', v_moved_suggestions
  );
