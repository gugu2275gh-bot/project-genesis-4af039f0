...
-- 1) Add opportunity_id to commissions
ALTER TABLE public.commissions ADD COLUMN IF NOT EXISTS opportunity_id uuid REFERENCES public.opportunities(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_commissions_opportunity_id ON public.commissions(opportunity_id);

-- 2) Replace contract-level auto-creation: now per service (opportunity)
...
BEGIN
  ...
  -- Iterate all services linked to this contract:
  --   a) the main opportunity (contracts.opportunity_id)
  --   b) opportunities linked via contract_leads -> leads
  FOR rec IN
    WITH linked_opps AS (
  ...
  LOOP
    ...
    IF EXISTS (
      ...
      SET base_amount = rec.base_amount,
          commission_amount = ROUND(rec.base_amount * COALESCE(commission_rate, v_rate), 2),
...
-- 3) Replace payment trigger: only ensure the commission for that service exists
