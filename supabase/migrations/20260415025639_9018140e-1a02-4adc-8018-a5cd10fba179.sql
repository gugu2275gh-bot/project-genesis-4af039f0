...
DECLARE
  ...
  v_moved_titular_links int := 0;
  v_moved_beneficiary_links int := 0;
BEGIN
  ...
  -- 9. Move contact data suggestions
  UPDATE contact_data_suggestions SET contact_id = p_target_contact_id
  WHERE contact_id = p_source_contact_id;
  GET DIAGNOSTICS v_moved_suggestions = ROW_COUNT;

  -- 10. Move beneficiary titular links (as titular)
  UPDATE beneficiary_titular_links SET titular_contact_id = p_target_contact_id
  WHERE titular_contact_id = p_source_contact_id
    AND beneficiary_contact_id != p_target_contact_id;
  GET DIAGNOSTICS v_moved_titular_links = ROW_COUNT;
  -- Remove any remaining links that would be self-referencing or duplicates
  DELETE FROM beneficiary_titular_links WHERE titular_contact_id = p_source_contact_id;

  -- 11. Move beneficiary titular links (as beneficiary)
  UPDATE beneficiary_titular_links SET beneficiary_contact_id = p_target_contact_id
  WHERE beneficiary_contact_id = p_source_contact_id
    AND titular_contact_id != p_target_contact_id;
  GET DIAGNOSTICS v_moved_beneficiary_links = ROW_COUNT;
  -- Remove any remaining links that would be self-referencing or duplicates
  DELETE FROM beneficiary_titular_links WHERE beneficiary_contact_id = p_source_contact_id;

  -- 12. Move beneficiary links (contacts linking to source as principal)
  UPDATE contacts SET linked_principal_contact_id = p_target_contact_id
  WHERE linked_principal_contact_id = p_source_contact_id;

  -- 13. Optionally update phone/email on target
  ...
  -- 14. Merge payment_notes (append source notes to target)
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'moved_titular_links', v_moved_titular_links,
      'moved_beneficiary_links', v_moved_beneficiary_links
    )
  );

  -- 16. Delete source contact
