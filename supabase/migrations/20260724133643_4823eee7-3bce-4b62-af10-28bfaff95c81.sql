...
BEGIN
  ...
  -- Base = valor pago; IVA é adicionado por cima
...
REVOKE EXECUTE ON FUNCTION public.auto_create_invoice_on_payment_confirmed() FROM anon;

-- Recalcular faturas existentes: base = total anterior, IVA = base * rate, total = base + IVA
UPDATE public.invoices
SET amount_without_vat = ROUND(total_amount, 2),
    vat_amount = ROUND(total_amount * COALESCE(vat_rate, 0.21), 2),
    total_amount = ROUND(total_amount + total_amount * COALESCE(vat_rate, 0.21), 2)
