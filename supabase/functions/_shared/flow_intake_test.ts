...
// --- "tenho 50 anos, tio na europa e moro em paris" -------------------------

import { countryFromCity } from './flow-intake.ts'

Deno.test('cidade conhecida define o país de residência', () => {
  assertEquals(countryFromCity('Paris'), 'França')
  assertEquals(countryFromCity('em Lisboa'), 'Portugal')
  assertEquals(countryFromCity('Madrid'), 'Espanha')
  assertEquals(countryFromCity('Xurupita'), '')
})

Deno.test('Paris: país deduzido da cidade e nada de presença na Espanha', () => {
  const src = extractionToSourceValues_pais(
    {
      age: 50,
      city: 'Paris',
      eu_family: 'tio na europa',
      in_spain: 'nao',
      confidence: { age: 1, city: 1, eu_family: 1, in_spain: 0.9 },
    } as any,
    0.7,
    new Date(),
    { message: 'tenho 50 anos , tio na europa e moro em paris' },
  )
  assertEquals(src.age, '50')
  assertEquals(src.residence_country, 'França')
  assertEquals(src.eu_family, 'sim')
  // A IA deduziu "não está na Espanha" só pelo endereço: valor descartado.
  assertEquals(src.in_spain, undefined)
})

Deno.test('resposta direta sobre estar na Espanha continua valendo', () => {
  const src = extractionToSourceValues_pais(
    { in_spain: 'nao', confidence: { in_spain: 1 } } as any,
    0.7,
    new Date(),
    { message: 'não' },
  )
  assertEquals(src.in_spain, 'nao')
})
