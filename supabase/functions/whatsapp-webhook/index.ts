...
function isQuestionAboutFullName(question: string): boolean {
  const n = normalizeForLanguageChecks(question)
...
function isAutoGeneratedContactName(name: string | null | undefined, whatsappProfileName: string | null | undefined, phoneNumber: string): boolean {
...
// R3: denylist — pronouns, family, places, common phrases that look like 2-word
// answers but are NOT a person's name. Prevents backfilling contact.full_name
// with garbage like "minha mãe" or "São Paulo".
const FULL_NAME_DENYLIST_PATTERNS: RegExp[] = [
  ...
  // Common Brazilian/Spanish/EU cities and countries appearing alone
  ...
  // Pure numbers / dates / generic words
  /^\d/, 
]

function isLikelyFullNameAnswer(text: string): boolean {
  const raw = text.trim()
  ...
  // the answer must be the immediate reply to a name question (handled by caller).
  return alphaWords.length >= 2
}

function findExplicitFullNameAnswer(
...
--- FIM DA INSTRUÇÃO ESPECIAL ---`
        ...
        // o LLM sempre veja os dados-chave já capturados do contato.
        {
          ...
          const pinName = isAutoGeneratedContactName(contact.full_name, message.name, phoneNumber) ? '' : (contact.full_name || '')
        ...
        if (explicitNameFromHistory && isAutoGeneratedContactName(contact.full_name, message.name, phoneNumber)) {
          const { error: nameBackfillError } = await supabase
            .from('contacts')
            .update({ full_name: explicitNameFromHistory })
            .eq('id', contact.id)
        ...
        if (currentMessageAsName && isAutoGeneratedContactName(contact.full_name, message.name, phoneNumber)) {
          const explicitCurrentName = String(messageForAI).trim()
          const { error: currentNameUpdateError } = await supabase
            .from('contacts')
            .update({ full_name: explicitCurrentName })
            .eq('id', contact.id)
        ...
        const knownName = isAutoGeneratedContactName(contact.full_name, message.name, phoneNumber) ? '' : contact.full_name || ''
        ...
        const nameMissing = isAutoGeneratedContactName(contact.full_name, message.name, phoneNumber)
